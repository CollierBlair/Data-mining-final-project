...
def overlay_image(original, recon):
    original = original.squeeze()
    recon = recon.squeeze()

    # normalize (important for visibility)
    original = original / (original.max() + 1e-8)
    recon = recon / (recon.max() + 1e-8)

    rgb = np.zeros((28, 28, 3))
    rgb[..., 1] = original   # GREEN
    rgb[..., 0] = recon      # RED
...
def plot_reconstructions(model, dataset, device, save_path, n=10):
    ...
    for col in range(n):
        img, label = dataset[col]
        img_t = img.unsqueeze(0).to(device)

        _, recon, _ = model(img_t, torch.tensor([label], device=device))

        orig_np = img.cpu().numpy()
        recon_np = recon.cpu().view(28, 28).numpy()

        # 🔥 overlay version (bottom row)
        overlay = overlay_image(orig_np, recon_np)

        # top row = original (grayscale)
        axes[0, col].imshow(orig_np.squeeze(), cmap="gray")
    ...
    plt.suptitle("CapsNet Reconstructions (Overlay)", fontsize=11)
    plt.tight_layout()
...
def plot_perturbations(model, dataset, device, save_path, digit_class=3):
    ...
    for dim in range(caps_dim):
        for col, delta in enumerate(perturbations):
            ...
            recon = model.decoder(
                perturbed_v,
                torch.tensor([digit_class], device=device)
            )
